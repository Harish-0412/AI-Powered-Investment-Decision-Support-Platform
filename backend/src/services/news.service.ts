...
const NEWS_API_KEY = "984372f759f5467d8e8d6aae6c844e48";
...
export const getTopMarketNews = async (category = "business", country = "us"): Promise<NewsArticle[]> => {
  const cacheKey = `news:top:financial:${category}:${country}`;
  const cached = await cacheGet(cacheKey);
  if (cached) return cached as NewsArticle[];

  // Using 'everything' with financial keywords for better stock market news
  const response = await fetch(
    `${BASE_URL}/everything?q=finance+stocks+market+investing&sortBy=publishedAt&language=en&apiKey=${NEWS_API_KEY}`
  );
